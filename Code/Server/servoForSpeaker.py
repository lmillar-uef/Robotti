...
class HardwareServo:
    def __init__(self, pcb_version):
        ...
        if self.pcb_version == 1:
            ...
            #self.pwm_gpio19 = HardwarePWM(pwm_channel=2, hz=50, chip=0)  # Initialize PWM for GPIO 13 on chip 0
            ...
            #self.pwm_gpio19 = HardwarePWM(pwm_channel=2, hz=50, chip=0)  # Initialize PWM for GPIO 13 on chip 2
        ...
        #self.pwm_gpio19.start(0)  # Start PWM for GPIO 13 with 0% duty cycle
    ...
    def setServoPwm(self, channel, angle, freq=0):
        # Set the PWM duty cycle for the specified channel and angle
        if channel == '0':
            duty = self.map(angle, 0, 180, 0, 100)  # Map angle to duty cycle
            self.setServoDuty(channel, duty)  # Set duty cycle for GPIO 12
            if freq !=0:
                self.setServoFrequency('0', freq)
        elif channel == '1':
            duty = self.map(angle, 0, 180, 0, 100)  # Map angle to duty cycle
            self.setServoDuty(channel, duty)  # Set duty cycle for GPIO 13
            if freq != 0:
                self.setServoFrequency('1', freq)
...
class Servo:
    ...
    def setServoAngle(self, channel, angle, freq):
        # Set the angle for the specified channel
        angle = self.angle_range(str(channel), int(angle))  # Ensure the angle is within the valid range
        self.pwm.setServoPwm(str(channel), int(angle), int(freq))  # Set the angle for the specified channel
